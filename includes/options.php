...
class College_Green_options {
	...
	public static function register ( $wp_customize ) {
   	...
   	public static function header_output() {
		...
		foreach ( self::$settings as $section_id => $section ) {
			foreach ( $section['fields'] as $setting_id => $setting ) {
				if ( isset( $setting['css'] ) && $setting['css'] && ( get_theme_mod( $setting_id, false ) || isset( $setting['setting']['default'] ) ) ) {
					$output[] = self::generate_css(
						$setting['css']['selector'],
						$setting['css']['style'],
						get_theme_mod( $setting_id, false ) ? get_theme_mod( $setting_id, false ) : $setting['setting']['default'],
